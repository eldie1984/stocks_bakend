...
stocks=pd.read_sql('''SELECT * FROM dg_stocks.follow
where  follow=1  ''',
                 con=engine)
...
def first_ask_bid(currency_pair, t):
    ...
    yahoo_symbol=stocks[stocks['symbol']==currency_pair]['rava'].values[0]
...
def replace_row(currency_pair, bid, ask):
    ...
    if hora >10 and hora<18 and week_day <6:
        ppi_symbol=stocks[stocks['symbol']==currency_pair]['ppi'].values[0]
